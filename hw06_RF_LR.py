...
# 選擇特定的列作為訓練特徵矩陣 X_train2
X_train2 = X_train.iloc[:, [1, 2]]

# 創建 StratifiedKFold 交叉驗證分割
...
for i, (train, test) in enumerate(cv):
    # 訓練模型並獲取預測概率
    probas = lg.fit(X_train2.iloc[train],
...
plt.xlim([-0.05, 1.05])
plt.ylim([-0.05, 1.05])
plt.xlabel('LR-False positive rate')
plt.ylabel('LR-True positive rate')
...
for k, (train, test) in enumerate(kfold):
    ...
    score = lg.score(X_train.iloc[test], y_train.iloc[test])
    scores.append(score)
